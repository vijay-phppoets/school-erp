        ...
        <div class="box box-primary">
             <div class="box-header with-border" >
                ...
                <div class="box-tools pull-right">
                  <?= $this->Html->link('Student Edit',['controller'=>'Students','action'=>'editStudent',$student_id,'FeeReceipts','hostelFee'],['class'=>'btn btn-warning','style'=>'color:#FFF !important;']) ?>
                </div>
            </div>
            <div class="box-body">
            <?= $this->Form->create($feeReceipt,['class'=>'FormSubmit','id'=>'ServiceForm']) ?>
                <?= $this->Form->control('fee_category_id',['label' => false,'type'=>'hidden','value'=>6])?>
                ...
                <table class="table table-bordered" style="text-align: center;">
                    ...
                    foreach ($FeeData as $FeeDatas) {
                        $y=1;
                        echo"<tr> <th>".$FeeDatas->fee_type->name."</th>";
                        foreach ($FeeDatas->fee_type_master_rows as $key => $value) {
                            echo"<th>". $this->Form->control('fee_type_master_rows['.$z.'][fee_type_master_row_id]', ['type' => 'checkbox','label'=>false,'class'=>'checkDisable','uncheck'=>'removeDisable'.$value->id,'value'=>$value->id])." Installment ".$this->Number->ordinal($y)."</th>";                                    
                            $y++;
                            $z++;
                        }
                        echo"</tr>";
                        ...
                        foreach ($FeeDatas->fee_type_master_rows as $key => $value) {
                            ...
                            echo "
                                ...
                                $this->Form->control('fee_type_master_rows['.$x.'][amount]',[
                                    'label' => false,
                                    'class'=>'form-control input-small amountValid removeDisable'.$value->id,
                                    'placeholder'=>'Amount',
                                    'type'=>'text',
            ...
            <tbody>
                ...
                foreach ($SubmittedFee as $Submittedlist) {
                    ...
                    foreach ($Submittedlist->fee_receipt_rows as $key => $value) { 
                        $FeeName[]=$value->fee_type_master_row->fee_type_master->fee_type->name.' ('.$value->amount.')';
                    } 
                    ...
                    <tr>
                        ...
                        <td> 
                            <?= $this->Html->link('<i class="fa fa-print"></i>',['controller'=>'FeeReceipts','action'=>'receiptPrint','FeeReceipts','hostelFee',$receipt_id,$student_info_id],['escape'=>false,'class'=>'btn btn-primary btn-xs']) ?>
...
{ 
    ...
    <div id="deletemodal<?php echo $Submittedlist->id; ?>" class="modal fade" role="dialog">
        <div class="modal-dialog modal-md" >
            <?= $this->Form->create('from',['url'=>['action'=>'delete','hostelFee',$receipt_id,$student_info_id]]) ?>
                <div class="modal-content">
                  <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal">&times;</button>

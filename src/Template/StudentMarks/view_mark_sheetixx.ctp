...
<div class="portlet light ">
                ...
                <div class="box-body">
                    <div class="a1">
						...
						<div  class="top-css1" style="font-size:21px; margin-top:0px; text-align:center;">
                                               ...
                                               <p>Session : <?php echo $sy_name;?></p></strong>
                                            </div>
                                        </td>
                                        <td class="mb-no-width" width="15%">
                                            <?php //echo $this->Html->image('cbselogo1.png', array('width' => '140','height'=>140)); ?>
    ...
    border-left: unset;">
                                    <tbody>
										...
										<th <?= $rowSpan!=0?'rowspan='.$rowSpan:'' ?> <?= $colSpan > 0?'colspan='.$colSpan:'' ?> height="15px" <?php if($last==0){?>width="8%"<?php }else{?>width="8%"<?php }?>><p>
                                                        <?= $section['name'] ?> </p>
                                                ...
                                                <?php if ($i == 0 && $marks_type != 'Grade'): ?>
                                                    <th rowspan="<?= $maxDepth ?>" width="8%"><p> Over All Total</p></th>
                                       ...
                                       <!-- <?php if ($marks_type != 'Grade'): ?>
                                        ...
                                        <?php $infogread=($infogread->toArray()); 
                                            ...
                                            { 
												...
												$pt_nicheto=0;$ot_pt=0;$total_sub_av_gnot=0;$ot_totalsubavgnot=0;
                                                foreach ($scholastic_subjects as $sub_key => $scholastic_subject):  ?>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <?php else: ?>
                                                            ...
                                                            <?php $total=0; ?>
                                                            <!-- print marks -->
															<?php $i=1;$totlessANNUAL=0;$ptbenottotlessANNUAL=0; $xyz=0;$arraymarks=[];$pt=0;$arraymarkss=0;$totlessNOTEBOOK=0;$ptbenotNOTEBOOK=0;$totlessSubjectEnrichment=0;$ptbenotSubjectEnrichment=0;?>
												...
												@$examid[$exam['id']]=1;		
																...
																if(strtoupper($result_row['exam_master']['name'])=="ANNUAL")
																{	
if(strtoupper($scholastic_subject['name'])!="COMPUTER")
																{															
												@$totalAnnualdatasmunber+=$result_row->obtain;
												@$totlaAnnualdatasmax+=$result_row->total;
													
																}
																}
										if(strtoupper($result_row['exam_master']['name'])=='Annual' || strtoupper($result_row['exam_master']['name'])=='ANNUAL' || strtoupper($result_row['exam_master']['name'])=='ANNUAL')
									{
										$totlessANNUAL=round($result_row->obtain);
										$ptbenottotlessANNUAL=$result_row->total;
										//pr($ptbenottotlessANNUAL);die;
									}												
																							}
																							else{
                              ...
                              if(strtoupper($result_row['exam_master']['name'])=='PT1' || strtoupper($result_row['exam_master']['name'])=='PT-1' || strtoupper($result_row['exam_master']['name'])=='PT-I')
									{
										array_push($arraymarks,round($best_of_number));
										$ptbest=$result_row->number_of_best;
										
									}if(strtoupper($result_row['exam_master']['name'])=='PT2' || strtoupper($result_row['exam_master']['name'])=='PT-2' || strtoupper($result_row['exam_master']['name'])=='PT-II')
									{
										array_push($arraymarks,round($best_of_number));
										
									}
									if(strtoupper($result_row['exam_master']['name'])=='PT3' || strtoupper($result_row['exam_master']['name'])=='PT-3' || strtoupper($result_row['exam_master']['name'])=='PT-III')
									{
										array_push($arraymarks,round($best_of_number));
										sort($arraymarks);
										array_shift($arraymarks);
										foreach($arraymarks as $arrayss)
										{
											$arraymarkss +=$arrayss;
										}
										
										$pt = round($arraymarkss/2);
									}
									if(strtoupper($result_row['exam_master']['name'])=='NOTEBOOK' || strtoupper($result_row['exam_master']['name'])=='NoteBook' || strtoupper($result_row['exam_master']['name'])=='Note Book')
									{
										
										$totlessNOTEBOOK=round($best_of_number);
										//pr($totlessNOTEBOOK);die;
										$ptbenotNOTEBOOK=$result_row->number_of_best;
										
									}
									if(strtoupper($result_row['exam_master']['name'])=='Subject Enrichment' || strtoupper($result_row['exam_master']['name'])=='SubjectEnrichment' || strtoupper($result_row['exam_master']['name'])=='SUBJECT ENRICHMENT')
									{
										$totlessSubjectEnrichment=round($best_of_number);
										$ptbenotSubjectEnrichment=$result_row->number_of_best;
										//pr($totlessSubjectEnrichment);die;
									}
												...
												@$totleoptentotle[$exam['id']]+=$result_row->number_of_best;
                                                            ...
                                                            <?php $subject_totals=$totlessANNUAL+$totlas;$totlessss=$ptbenottotlessANNUAL+$bestofdskkl; if ($marks_type != 'Grade'): ?>
                                                            <td><?= $subject_total ?> / <?= $total ?></td>
                                                                <?php if ($marks_type != 'Number'): ?>
                                                                    <th style="text-align: center;">
																	<?php $subject_per = $subject_total*100/$total;
																	       $subject_per=round($subject_per);
																		    if($subject_per>=33){
							...
							return ['sublimenterysubject'=>@$sublimenterysubject,'countofsulimetri'=>@$countofsulimetri,'totlaAnnualdatasmax'=>@$totlaAnnualdatasmax,'totalAnnualdatasmunber'=>@$totalAnnualdatasmunber,'examcol'=>@$examid,'totleoptentotle'=>@$totleoptentotle,'totleopten'=>@$totleopten,'pt_nicheto'=>@$pt_nicheto,'ot_pt'=>$ot_pt,'total_sub_av_gnot'=>$total_sub_av_gnot,'ot_totalsubavgnot'=>$ot_totalsubavgnot];					
                                            }
...
-->			<?php if($marks_type !== 'Grade'){?>		 <tr class="header_font">
                                            ...
                                            <?php $prj=0; $studentMarks = 0; $totalMarks = 0;  $grand_total=0; if($ccf['examcol']) { foreach ($ccf['examcol'] as $sub_key => $scholastic_subject){
												?>
                                               ...
                                               <?php
                                                    ...
                                                    $studentMarks += $ccf['totleopten'][$sub_key];
                                                    $totalMarks   += $ccf['totleoptentotle'][$sub_key];
											  
											 @$OverAllTotalmax= $ccf['totlaAnnualdatasmax']+$ccf['ot_totalsubavgnot'];@$OverAllTotalnum=$ccf['totalAnnualdatasmunber']+$ccf['total_sub_av_gnot'];
                                           $prj++; } }?>
                                           <th><p><?php echo @$studentMarks;?>/<?php echo @$totalMarks; ?></p></th> 
                                           <th><p ><?php  @$totlepersent=$studentMarks*100/ $totalMarks;
...
<!-- CO-SCHOLASTIC--><?php  if($marks_type != 'Grade'){?>
					...
					<td width="70%">
						<table width="100%" border="1" cellspacing="0" cellpadding="0" >
							...
							<tr >
								...
								<?php 
						if($ccf['countofsulimetri']>0 && $ccf['countofsulimetri']<=1)
									{
										
										echo implode(" , ",$ccf['sublimenterysubject']);
									}
									else{
										echo '-';
									}
							...
							<tr>
								...
								<?php  
								if($ccf['countofsulimetri']<1)
								{
									echo 'PASS';
								}
								elseif($ccf['countofsulimetri']>0 && $ccf['countofsulimetri']<2)
								{
									echo 'SUPPLEMENTARY';
								}
								else{
							...
							<tr>
                                  ...
                                  if($ccf['countofsulimetri']<1)
								{
                                    ...
                                    //  $overallpass=$OverAllTotalnum*100/$OverAllTotalmax;
                                     $overallpass=$studentMarks*100/$totalMarks;
...
<div class="row row-eq-height">
           ...
           <table cellspacing="0px" cellpadding="0px" border="1" id="sample_1" style="text-transform: uppercase;margin-top: -3px;width: 100.2%;border-left: unset;" >
										...
										<th class="header_font second_color" style="width: 25%;text-align: center !important;padding: 6px;font-weight: 600 !important;font-weight: 600!important;">Sr.No.</th>
													...
													<th class="header_font second_color" style="width: 12%;text-align: center !important;padding: 6px;font-weight: 600 !important;font-weight: 600!important;" >Report  I</th>
													<th class="header_font second_color" style="width: 12%;text-align: center !important;padding: 6px;font-weight: 600 !important;font-weight: 600!important;" >Report  II</th>
													<th class="header_font second_color" style="width: 12%;text-align: center !important;padding: 6px;font-weight: 600 !important;font-weight: 600!important;" >Report  III</th>

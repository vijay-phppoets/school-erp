                        ...
                        <thead>
                            ...
                            <?php for ($i=0; $i < $itteration; $i++) { ?>
                                <tr>
									...
									<th class="total totalg" rowspan="<?= $maxDepth ?>">Annual PASS</th>
                                </tr>
                    ...
                    <?php if (isset($students)): ?>
                        <tbody>
                            ...
                            <?php  $j=0; $i = 0; foreach ($students as $key => $student): $i++;?>
                                ...
                                <?php endif ?>
                                        <?php $add_exam.=$j; ?>
                                        <td class="marks" exam="<?= $add_exam ?>" max-mark = "<?= $student['sub_max'] ? $student['sub_max'] : $student['max_marks'] ?>" bestof="<?=$student['number_of_best'] ?>" examname="<?= $student['examname'] ?>" student_number="<?= $student['student_number'] ?>">
...
$(document).ready(function() {
	...
	$('.marks',this).each(function(e){
		  var aaa=$(this).attr('student_number');
		 if(aaa=='' || aaa=='A' || aaa=='a')
		 {
			 aaa=0;
		 }
		  var markersmin=$(this).attr('examname');
		
		  var main_exam_bestofs=$(this).attr('bestof');
		  var maxqmark=$(this).attr('max-mark');
		
		  if(markersmin=='PT1' || markersmin=='PT2'|| markersmin=='PT3')
		  {
		  ...
		  subjectenrichment +=parseInt(aaa);
		   ...
		   maxwmarksubjecte +=parseInt(maxqmark);
			  ...
			  comantotle +=parseInt(aaa);
			   ...
			   bestofannual = parseInt(main_exam_bestofs);
		  }
	 }); 
	     if(!gtotals==0)
		 {
	    ...
	    var obtain =gtotals+notbookmark+subjectenrichment;
	   var totalassssmax = bestofpa1+bestofnotbook+bestofsubjecte;
	
	  
	required = Math.ceil(obtain/totalassssmax*100);
	requiredsss = Math.ceil(34*totalassssmax/totalassssmax);
	requiredsssnew = Math.ceil(34*totalassssmax/100);
	
	requiredannual=  Math.ceil(comantotle);
	requiredannualnew= Math.ceil((33*maxwmarkscomantotle)/100);
	var newannualpass=requiredannual-requiredannualnew;
	newannualpass= Math.abs(newannualpass);
	
	//alert(obtain);
	//alert(totalassssmax);
	//alert(required); 
	//alert(requiredsss);
	 //var passforpass1=  obtain-requiredsssnew;
	// var passforpass=passforpass1*100/totalassssmax;
	//passforpass= Math.abs(passforpass);
	 var passforpass=  requiredsss-obtain;	
			...
			if(requiredannualnew<=requiredannual){
				var textpassfail='Pass';
				var colorss='background:greenyellow';
				$(this).closest('tr').append('<td class=\'mca\' style='+colorss+'>'+textpassfail+'</td>');
			}else{
				var textpassfail='Fail';
				var colorss='background:red';
				$(this).closest('tr').append('<td class=\'mca\' style='+colorss+'>'+textpassfail+' ('+newannualpass+')</td>');
			}

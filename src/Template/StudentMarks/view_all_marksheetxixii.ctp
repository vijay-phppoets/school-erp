...
$totleopten=[]; 
															$totleoptentotle=[]; 
															$examid=[]; 
															$ot_pt=0;$pt_nicheto=0;
															$total_sub_av_gnot=0;$ot_totalsubavgnot=0;
															$OverAllTotalnum=0;@$OverAllTotalmax=0;
															$totlaAnnualdatasmax=0;$totalAnnualdatasmunber=0;
															$countofsulimetri=0; 
   $sublimenterysubject=[]; 
   $componentData=[];	
	?>
...
<?php $j=0;?> 
                                                <?php foreach ($examArray as $key => $section): ?>
													...
													?>
													<?php if($i!=0){?>
                                                    <th <?= $rowSpan!=0?'rowspan='.$maxDepth:'' ?> <?= $colSpan > 0?'colspan='.$colSpan:'' ?> height="15px"  <?php if($last==0){?>width="15%"<?php }else{?>width="8%"<?php }?>>
                                       ...
                                       <!-- <?php if ($marks_type != 'Grade'): ?>
                                        ...
                                        <?php /* $infogread=($infogread->toArray());  */
                                            ...
                                            $subject_elevet =$subjectArr1[$student['id']];
												...
												<?php if(in_array($scholastic_subject['id'], $subject_elevet)){?>
                                                        ...
                                                        <?php if(empty($scholastic_subject['children'])):?>
                                                            ...
                                                            <?php foreach ($scholastic_subject['exams'] as $exam_key => $exam): ?>
                                                                <td style="font-size:14px;padding:10px">
                                                                    <?php foreach ($marks as $key => $results):
...
-->		
	...
	<?php   if($marks_type !== 'Grade'){ ?>		 <tr class="header_font">
                                            ...
                                            <?php $prj=0; $grand_total=0; if($examid) { foreach ($examid as $sub_key => $scholastic_subject){
												?>
                                               ...
                                               <?php
											    ...
											    @$OverAllTotalmax= $totlaAnnualdatasmax+$ot_totalsubavgnot;@$OverAllTotalnum=$totalAnnualdatasmunber+$total_sub_av_gnot;
												
                                          $prj++;  } }
										  $componentData['marks']=$OverAllTotalnum;
											$componentData['marksmax']=$OverAllTotalmax;
										  ?>
                                           <th style='font-size: 14px;'><p><?php echo @$OverAllTotalnum;?>/<?php echo @$OverAllTotalmax; ?></p></th> 
                                           <th style='font-size: 14px;'><p ><?php  @$totlepersent=$OverAllTotalnum*100/ $OverAllTotalmax;
...
<?php } $prrj=0;$rjpr=0;?>										
    ...
    border-left: unset;" >
										...
										<?php } ?>
													<th class="header_font second_color" style="width: 25%;text-align: center !important;padding: 6px;font-weight: 600 !important;font-weight: 600!important;">Sub Subject</th>
													<?php $prdk=count($subject['student_marks']);
													if($prdk==1)
													{
													?>

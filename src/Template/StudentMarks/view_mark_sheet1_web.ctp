...
-->		
                                        	...
                                        	<?php   if($marks_type !== 'Grade'){ 
                                             $exam_arrayyyyy = array(); 
                                             foreach ($scholastic_subject['exams'] as $exam_key => $exam)
                                             { 
                                                $exam_arrayyyyy[] = $exam['id'];
                                             } 
                                             

                                            ?>		 <tr class="header_font">
										
                                            <th ><?php //echo "<pre>"; print_r($exam_arrayyyyy);  ?><p style="text-align: left!important;padding-left: 5px;padding: 10px 0;">Total</p></th>
                                            <?php $grand_total=0;

                                             if($examid) 
                                            {     
                                                $count_exam = count($examid);    
                                                if($count_exam == 4){ ?>
                                                        <th></th>
                                                        <th></th>
                                                        <th></th>
                                                        <th></th>
                                                <?php } ?>
                                               <?php foreach ($examid as $sub_key => $scholastic_subject){ 
												?>
...
<?php $xyz=0; 
	foreach ($att as $attend_meeting){ 
		if($attend_meeting['student_id']==$student->student_id){  $xyz+=1; $prdk=$xyz;}
    }
?>
						
<?php  /* if($prdk==1) */ if(count($att) == 1)  { ?>
...
<?php }?>
	<tr>
        ...
        <?php foreach ($att as $attend_meeting): ?>
            <?php if($attend_meeting['student_id'] == $student->id): ?>
    ...
    <tr>
        ...
        <td><?php
            ...
            if(!empty((int)$attend_meeting['attend_meeting'])) echo round(($attend_meeting['attend_meeting']*100)/$attend_meeting['total_meeting'],2);

            else echo $attend_meeting['attend_meeting'];
            ...
            <td>
                <?php if(!empty((int) $attend_meeting['attend_meeting'])): ?>
                        <?php $per1=round(((int) $attend_meeting['attend_meeting']*100)/ (int) $attend_meeting['total_meeting'],2);
						if($per1>=90 && $per1<=100)
						{
							echo "Excellent";
							}
					...
					?>
                <?php else : ?>
                <?php echo $attend_meeting['attend_meeting']; ?>
                <?php endif; ?>
            </td>

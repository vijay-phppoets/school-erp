	...
	<div class="col-md-12">
		<div class="box box-primary">
			<div class="box-header with-border" >
				<label> Purchase Order Edit : <?= h($purchaseOrder->po_no) ?> </label>
			</div>
			<?= $this->Form->create($purchaseOrder, ['id'=>'ServiceForm','autocomplete'=>'off']) ?>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="col-sm-4">
							...
							'value'=>$purchaseOrder->transaction_date,'placeholder'=>'Transaction Date'])?>
                        </div>
				...
				<div class="row">
					<div class="col-sm-12" style="margin-top:10px;padding: 0px 33px 0px 33px;" id="main">
						<table class="table table-bordered" id="main_table">	
							...
							<tbody id="main_tbody">
								<?php $x=0; foreach($purchaseOrder->purchase_order_rows as $purchaseOrderRow){?>
								<tr class="main_tr">
									<td style="vertical-align: top !important;"><?php echo ++$x; ?></td>
									<td>
									<?php  $this->Form->hidden('id',['value'=>$purchaseOrderRow->id])?>
										<?php echo $this->Form->control('item_id',['options'=>$option,'class'=>'selectadd item_id ' ,'empty' => 'Select...','label'=>false,'required'=>'required','style'=>'width:100%','value'=>$purchaseOrderRow->item_id]); ?>
										<input type="hidden" class="purchase_order_id" value="<?php echo $purchaseOrderRow->id; ?>" /> 
									</td>
									<td id="staticParent">
										<?php echo $this->Form->control('quantity', ['label' => false,'placeholder'=>'Qty','class'=>'form-control input-sm quantity rightAligntextClass calc','required'=>'required','id'=>'child',
										'value'=>$purchaseOrderRow->quantity]); ?>	
									</td>
									<td>
										<?php echo $this->Form->control('rate',['class'=>'form-control input-sm rate numberOnly rightAligntextClass calc','placeholder'=>'Rate','label'=>false,'required'=>'required','value'=>$purchaseOrderRow->rate]); ?>	
									</td>		
									<td>
										<?php echo $this->Form->control('amount', ['style'=>'text-align:right','label' =>false,'class' => 'form-control input-sm  amount numberOnly','type'=>'text','value'=>0, 'readonly', 'tabindex' => '-1','value'=>$purchaseOrderRow->amount]);?>
									</td>
									<td>
										<?php
										if($x != 1){
							...
							<tfoot>
								<tr>
									...
									<td>
										<?php echo $this->Form->control('grand_total',['style'=>'text-align:right','label' => false,'class' => 'form-control input grand_total','type'=>'text','readonly'=>'readonly', 'tabindex' => '-1','value'=>$purchaseOrder->grand_total]); ?>
									</td>
									<td></td>
								</tr>
							</tfoot>
	...
	$(document).ready(function() {
		...
		function rename_rows()
		{
			...
			$('#main_table tbody#main_tbody tr.main_tr').each(function(){
				...
				$(this).find('td:nth-child(2) input').attr({name:'purchase_order_rows['+i+'][id]', id:'purchase_order_rows-'+i+'-id'
					});	
...
<table id="sample" style="display:none;"width="1500px">
	<tbody>
		<tr class="main_tr">
			...
			<td id="staticParent">
				<?php echo $this->Form->control('quantity', ['label' => false,'placeholder'=>'Qty','class'=>'form-control input-sm quantity rightAligntextClass calc numericOnly','required'=>'required','id'=>'child','oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>	
			</td>
			<td>

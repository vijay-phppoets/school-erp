				...
				<div class="row">
					<div class="col-md-12">
						<div class="col-sm-4">
                            ...
                            <?= $this->Form->control('transaction_date',['class'=>'datepicker form-control','label'=>false,'data-date-format'=>'dd-M-yyyy','value'=>$itemReturn->transaction_date,'placeholder'=>'Transaction Date','type'=>'text'])?>
                        </div>
						<div class="form-group col-md-4 employee hidden"  >
							...
							<?php echo $this->Form->control('employee_id',['options'=>$employees,'label' =>false,'class'=>'select2 ','empty'=> 'select...','required'=>'required','style'=>'width:100%','value'=>$itemReturn->employee_id]);?>
						</div>
						<div class="form-group col-md-4 student">
							...
							<?php echo $this->Form->control('student_id',['options'=>$students,'label' =>false,'class'=>'select2 ','empty'=> 'select...','required'=>'required','style'=>'width:100%','value'=>$itemReturn->student_id]);?>
						</div>
						<div class="col-md-4">
						<br/><br/>
				...
				<div class="row">
					<div class="col-sm-12" style="margin-top:10px;padding: 0px 33px 0px 33px;" id="main">
						<table class="table " id="main_table">	
							...
							<tbody id="main_tbody">
							<?php $x=0; foreach( $itemReturn->item_issue_return_rows as $ItemIssueReturnRows){?>
								<tr class="main_tr">
									<td style="vertical-align: top !important;"><?php echo ++$x; ?></td>
									<td>
									  <?php  $this->Form->hidden('id',['value'=>$ItemIssueReturnRows->id])?>
										<?php echo $this->Form->control('location_id',['options'=>$Dropdown,'class'=>' location_id','empty' => 'Select...','label'=>false,'required'=>'required','style'=>'width:100%','value'=>$ItemIssueReturnRows->location_id]); ?>
									</td>
									<td>
										<?php echo $this->Form->control('item_id',['options'=>$option,'class'=>' item_id ' ,'empty' => 'Select...','label'=>false,'required'=>'required','style'=>'width:100%','value'=>$ItemIssueReturnRows->item_id]); ?>
									</td>
									<td>
										<?php echo $this->Form->control('quantity', ['label' => false,'placeholder'=>'Qty','class'=>'form-control input-sm quantity rightAligntextClass calc','required'=>'required','value'=>$ItemIssueReturnRows->quantity,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>	
									</td>
									<td>
										<?php echo $this->Form->control('rate',['class'=>'form-control input-sm rate numberOnly rightAligntextClass calc','placeholder'=>'Rate','label'=>false,'required'=>'required','value'=>$ItemIssueReturnRows->rate,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>	
									</td>		
									<td>
										<?php echo $this->Form->control('amount', ['style'=>'text-align:right','label' =>false,'class' => 'form-control input-sm  amount numberOnly','type'=>'text','value'=>0, 'readonly', 'tabindex' => '-1','value'=>$ItemIssueReturnRows->amount,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]);?>
									</td>
									<td>
										<?php
										if($x != 1){
							...
							<tfoot>
								<tr>
									...
									<td>
										<?php echo $this->Form->control('grand_total',['style'=>'text-align:right','label' => false,'class' => 'form-control input grand_total','type'=>'text','readonly'=>'readonly', 'tabindex' => '-1','value'=>$itemReturn->grand_total,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>
									</td>
									<td></td>
								</tr>
							</tfoot>
	...
	$(document).ready(function() {
		...
		function rename_rows()
		{
			...
			$('#main_table tbody#main_tbody tr.main_tr').each(function(){
				...
				$(this).find('td:nth-child(2) input').attr({name:'item_issue_return_rows['+i+'][id]', id:'item_issue_return_rows-'+i+'-id'
					});	
...
<table id="sample" style="display:none;"width="1500px">
	<tbody>
		<tr class="main_tr">
			...
			<td>
				<?php echo $this->Form->control('quantity', ['label' => false,'placeholder'=>'Qty','class'=>'form-control input-sm quantity rightAligntextClass calc','required'=>'required','oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>	
			</td>
			<td>
				<?php echo $this->Form->control('rate',['class'=>'form-control input-sm rate numberOnly rightAligntextClass calc','placeholder'=>'Rate','label'=>false,'required'=>'required','oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>	
			</td>		
			<td>
				<?php echo $this->Form->control('amount', ['style'=>'text-align:right','label' =>false,'class' => 'form-control input-sm  amount numberOnly','type'=>'text','value'=>0, 'readonly', 'tabindex' => '-1','oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]);?>
			</td>
			<td>

...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $appointmentMaster->id],
                ['confirm' => __('Are you sure you want to delete # {0}?', $appointmentMaster->id)]
            )
        ?></li>
...
<div class="appointmentMasters form large-9 medium-8 columns content">
    <?= $this->Form->create($appointmentMaster) ?>
    <fieldset>
        <legend><?= __('Edit Appointment Master') ?></legend>
        <?php

	...
	<div class="col-md-12">
		<div class="box box-primary">
			<div class="box-header with-border">
					<label> Purchase Return Edit </label>
			</div>
				...
				<div class="row">
					<div class="col-md-12">
						<div class="col-sm-4">
                            ...
                            <?= $this->Form->control('data[date_from >=]',['class'=>'datepicker form-control','label'=>false,'data-date-format'=>'dd-M-yyyy','name'=>'transaction_date','value'=>$purchaseReturn->transaction_date,'placeholder'=>'Transaction Date'])?>
                        </div>
						<div class="form-group col-md-4" >
							<label class="control-label" >Vendors<span class="required" required name="vandors">*</span></label>
							<?php echo $this->Form->control('vendor_id',['options' =>$vendors,'label' =>false,'class'=>'select2 ','empty'=> 'select...','required'=>'required','style'=>'width:100%','value'=>$purchaseReturn->vendor_id]);?>
						</div>
					</div>
				</div>
				<div class="row">
					<div class="col-sm-12" style="margin-top:10px;padding: 0px 33px 0px 33px;" id="main">
						<table class="table " id="main_table">	
							...
							<tbody id="main_tbody">
							<?php $x=0; foreach($purchaseReturn->purchase_return_rows as $PurchaseReturnRows){ //pr($PurchaseReturnRows);Exit;?>
								<tr class="main_tr">
									<td style="vertical-align: top !important;"><?php echo ++$x; ?></td>
									<td>
									<?php  $this->Form->hidden('id',['value'=>$PurchaseReturnRows->id])?>
										<?php echo $this->Form->control('location_id',['options'=>$Dropdown,'class'=>'item_id','empty' => 'Select...','label'=>false,'required'=>'required','style'=>'width:100%','value'=>$PurchaseReturnRows->location_id]); ?>
										<input type="hidden" class="purchase_return_id" value="<?php echo $PurchaseReturnRows->id; ?>" />
									</td>
									<td>
										<?php echo $this->Form->control('item_id',['options'=>$option,'class'=>'item_id','empty' => 'Select...','label'=>false,'required'=>'required','style'=>'width:100%','value'=>$PurchaseReturnRows->item_id]); ?>
									</td>
									<td>
										<?php echo $this->Form->control('quantity', ['label' => false,'placeholder'=>'Qty','class'=>'form-control input-sm quantity rightAligntextClass calc','required'=>'required','value'=>$PurchaseReturnRows->quantity,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>	
									</td>
									<td>
										<?php echo $this->Form->control('rate',['class'=>'form-control input-sm rate numberOnly rightAligntextClass calc','placeholder'=>'Rate','label'=>false,'required'=>'required','value'=>$PurchaseReturnRows->rate,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>	
									</td>	
									<td>
										<?php echo $this->Form->control('amount', ['style'=>'text-align:right','label' =>false,'class' => 'form-control input-sm  amount numberOnly','type'=>'text','value'=>0, 'readonly', 'tabindex' => '-1','value'=>$PurchaseReturnRows->amount,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]);?>
									</td>
									<td>
										<?php
										if($x != 1){
							...
							<tfoot>
								<tr>
									...
									<td>
										<?php echo $this->Form->control('grand_total',['style'=>'text-align:right','label' => false,'class' => 'form-control input grand_total','type'=>'text','readonly'=>'readonly', 'tabindex' => '-1','value'=>0,'value'=>$purchaseReturn->grand_total,'oninput'=>"this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"]); ?>
								...
								<tr>
									...
									<td colspan="2">
										<?php echo $this->Form->control('remark', ['label' => false,'placeholder'=>'Comment Textbox for remark','class'=>'form-control input-sm quantity rightAligntextClass calc','required'=>'required','value'=>$purchaseReturn->remark,'style'=>'resize:none;']); ?>	
									</td>
								</tr>
							</tfoot>
						</table>
	...
	$(document).ready(function() {
		...
		function rename_rows()
		{
			...
			$('#main_table tbody#main_tbody tr.main_tr').each(function(){
				...
				$(this).find('td:nth-child(2) input').attr({name:'purchase_return_rows['+i+'][id]', id:'purchase_return_rows-'+i+'-id'
					});

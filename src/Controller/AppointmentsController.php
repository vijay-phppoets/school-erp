...
class AppointmentsController extends AppController
{
    public function beforeFilter(Event $event)
    {
        ...
        $this->Security->setConfig('unlockedActions', ['add','edit','index','report','appmtApproval']);
    ...
    {    //$login_type = $this->Auth->User('user_type');
        ...
        if($this->request->is(['post']))
        {
            ...
            $this->paginate = [
            'contain' => ['Students', 'Employees', 'AppointmentMasters'=>['Employees']
             ]];
            $appointments = $this->paginate($this->Appointments->find()->where([$where,'Appointments.created_by'=>$user_id]));
        }
        //pr($where);exit;
        ...
        foreach ($appointmentMasters as $appointmentMaster) {
            ...
            $appointmentMasterArr[]=['value' => $appointmentMaster->id,'text' => $appointmentMaster->employee->name];
            # code...
        } 
        
        $this->set(compact('appointment','appointments','students','appointmentMasters','employees','appointmentMasterArr'));
    }
    ...
    public function view($id = null)
    {
        $appointment = $this->Appointments->get($id, [
            'contain' => ['Students', 'Employees', 'AppointmentMasters']
        ]);
    ...
    public function add()
    {
        ...
        $students = $this->Appointments->Students->find('list');
        $employees = $this->Appointments->Employees->find('list');
        $appointmentList = $this->Appointments->AppointmentMasters->find()->where(['AppointmentMasters.is_deleted'=>'N'])->contain(['Employees']);
        $appointmentMasters=[];
        foreach ($appointmentList as $emlist) {
           $appointmentMasters[]=['value'=>$emlist->id,'text'=>$emlist->employee->name];
        }
         
        $this->set(compact('appointment', 'studentInfos', 'employees', 'appointmentMasters'));
    }
